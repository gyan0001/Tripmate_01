...
const SignupPage = () => {
  ...
  // Check if we're on Emergent preview (Google login only works there)
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-semibold text-slate-900">TripMate</h1>
          <p className="text-slate-500 text-sm mt-1">Your Kiwi Trip Planner</p>
        </div>

        {/* Card */}
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
          <h2 className="text-xl font-semibold text-slate-900 text-center mb-6">Create your account</h2>
          
          {/* Google Signup - Only show on Emergent preview */}
          {isEmergentPreview && (
            <>
              <button
                onClick={loginWithGoogle}
                className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors mb-4"
              >
        ...
        <p className="text-center text-slate-400 text-xs mt-6">
          Plan your perfect New Zealand adventure
        </p>
      </div>
    </div>
  );
